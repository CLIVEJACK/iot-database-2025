...
UPDATE Book SET bookname='테이터베이스 개론' WHERE bookid = 99;
 
SELECT bookname AS 'bookname2'FROM Book WHERE bookid = 99;
SAVEPOINT b;

UPDATE Book SET bookname='테이터베이스 개론 및 실습'WHERE bookid = 99;
...
-- START TRANSACTION에서 COMMIT까지는 트랜잭션이 살아있음
-- SAVEPOINT는 트랜잭션 상에서 ROLLBACK TO로 이동가능
-- ROLLBACK은 전부 취소
START TRANSACTION;
UPDATE Book SET bookname='테이터베이스 개론 및 실습2'WHERE bookid = 99;
